...
NDTFrame::NDTFrame(Vector3d trans, unsigned short width, unsigned short height, double cell_side, bool init_cell_windows, NDTPSOConfig config
  ...
  this->cells = vector<NDTCell>(this->numOfCells, NDTCell(init_cell_windows));
...
#ifdef OPENCV_FOUND
  if (save_image) {
    sprintf(output_filename, "%s-w%d-%dp%di-%dx%d-c%.2f-%dppm.png", filename, NDT_WINDOW_SIZE, this->s_config.psoConfig.populationSize,
